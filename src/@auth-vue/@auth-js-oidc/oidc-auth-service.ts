...
export abstract class OIDCAuthService<T extends OIDCAuthSettings = OIDCAuthSettings> {
    ...
    /**
     * @see {@link OIDCAuthManager}
     */
    public getManager(): OIDCAuthManager {
        return this.#manager;
    }
    ...
    public async runGuard(options?: AuthGuardOptions): Promise<string | boolean> {
        const authGuardValidator = new OIDCAuthGuard(this.#manager);
        return authGuardValidator.validate(options);
    }
}
